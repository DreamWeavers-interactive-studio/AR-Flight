...
public class EnemyWaveManager : MonoBehaviour
{
    ...
    private void SpawnEnemyWave()
    {
        for (int i = 0; i < waveData.numberOfEnemies; i++)
        {
            ...
            float rotationDeviation = Random.Range(-10f, 10f);
            lookRotation *= Quaternion.Euler(rotationDeviation, rotationDeviation, rotationDeviation);

            GameObject enemy = Instantiate(randomEnemyPrefab, randomPosition, lookRotation);
        }
    }
}
